 ...
 *
 * Architecture: ES module with window.MenuService for backward compatibility
 */
 ...
 * @returns {Promise<Object>} {success: boolean, error: string|null}
 */
export function updateMenuOrder(orderedIds) {
  if (!FirebaseService) {
    return Promise.resolve({
      success: false,
      error: 'FirebaseService not available'
...
// ---------------------------------------------------------------------------
// MenuService object for backward compatibility
// ---------------------------------------------------------------------------
